...
def test_struct_type_helpers(backend):
...
def test_struct_array_call(backend):
    dtype = np.dtype([("x", np.int32), ("y", np.float64)], align=True)
...
def test_struct_class_reuse(backend):
...
def test_struct_scalar(backend):
    dtype = np.dtype([("x", np.int32), ("y", np.float64)], align=True)
...
def test_struct_nested_array(backend):
    n = 2
    m = 3
